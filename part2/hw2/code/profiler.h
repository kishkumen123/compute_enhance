...
struct ProfileBlock{
    ...
    ProfileBlock(String8 label_, u32 anchor_index_, u64 bytes){
...
#define begin_timed_scope(NAME) begin_timed_bandwith(NAME, 0)
...
print_anchor_data(u64 total_cpu_elapsed, u64 cpu_freq){
    for(u32 anchor_index = 0; anchor_index < ArrayCount(profile_anchors); ++anchor_index){
        ...
        if(anchor->bytes_processed){
            ...
            print("  %.3fmb at %.2fgb/s", megabtes, gigabytes_per_second);
        }
        print("\n");
    }
}
